...
def parse_option():
    version = "%%prog %s" % schediag.__version__
    ...
    configpath = options.config or "%s/.schediagrc" % os.environ.get('HOME')
    if os.path.isfile(configpath):
        config = SafeConfigParser()
        config.read(configpath)

        if config.has_option('schediag', 'fontpath'):
            fontpath = config.get('schediag', 'fontpath')
